...
//	Optional<User> findById(Long id);
	...
	// User만 사용할 거라 User에 OneToMany인 UserRole도 함께 영속화
//	@Query("select u from User u join fetch u.userRoles where u.email = :email")
//	Optional<User> findByEmail(@Param("email") String email);
	
	// JPQL + EAGER -> select 2건 
	// JPQL은 email로 User 엔티티만 영속화, OneToMany인 UserRole도 추가로 영속화 
//	@Query("select u from User u where u.email = :email")
//	Optional<User> findByEmail(@Param("email") String email);
	 
	// JPQL + join + EAGER -> select 2건 
	// JPQL이 Useroles와 join을 수행하지만, UserRoles를 사용하지 않기 때문에
	// 	UserRoles는 단순 JPQL 수행 용도. email로 User 엔티티만 영속화, User에 OneToMany인 UserRole도 추가로 영속
//	@Query("select u from User u join u.userRoles where u.email = :email")
//	Optional<User> findByEmail(@Param("email") String email);
	
	// JPQL + join + LAZY -> 예외발생 : open-in-view가 false일 때 (true면 오류 없음)
	// open-in-view가 false이므로 JPA 영속성 컨텍스트는 repository 단계에서 종료
