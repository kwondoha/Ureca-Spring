...
public class MyUserDetailsService implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
		...
		if( optionalUser.isPresent() ) {
			...
			// _5 기준 ListUserRole -> String[] roleStrArray를 만들어서 메소드로 전달
			// 내부적으로 ROLE_CUSTOMER 형식으로 ROLE_prefix 처리하고 내부적으로 GrantedAuthority Collection 처리
			// org.springframework.security.core.userdetails.User.builder()..roles(roleStrArray)
			// 이제 MyUserDetails 객체를 사용하므로 우리가 직접 GrantedAuthority Collection을 만들어야 함
			List<SimpleGrantedAuthority> authorities = listUserRole
														.stream()
